...
const detect = ( stalker_id, effector_id, effector_data ) => { 
    // stalker_id and effector_id is a document id in Firebase. Effector_data is a copy  of document data for artifact-anomaly-whatever
...
const interact = ( stalker_id, stalker_data, effector_data, distance ) => { 
    // stalker_id is a document id in Firebase. Effector_data is a copy of document data for artifact-anomaly-whatever, distance is in meters
    ...
    await docSnap.get().then(function( thisDoc ) {
        if( thisDoc.exists) {
            if( thisDoc.data()["is_alive"] ) {
                // console.log(stalker_id + " is HIT! "+effect_rad+":"+effect_tox+":"+effect_phys);
                new_health = thisDoc.data()["health"];
                level_rad = thisDoc.data()["damage_rad"];
                level_tox = thisDoc.data()["damage_tox"]
                if( effect_rad > 0 ) { level_rad += effect_rad; new_health -= level_rad; } // radiation damage only if defined
                if( effect_tox > 0 ) { level_tox += effect_tox; new_health -= level_tox; } // toxic damage only if defined
                if( level_rad > 20 ) { level_rad = 20; }
                if( level_tox > 20 ) { level_tox = 20; }
                new_health -= effect_phys;
                ...
                retval = {"health": new_health, "is_alive": new_is_alive, "damage_rad": level_rad, "damage_tox": level_tox};
            }
        }
    });
    // apply effects
...
const activate = async ( object_id, new_active ) => { // set active flag for object (true/false)
    let docSnap = await firestore().collection('entities').doc(object_id)
    await docSnap.get().then(function( thisDoc ) {
        if( thisDoc.exists) {
            if( typeof thisDoc.data()["is_active"] !== 'undefined' ) {
                const res = docSnap.update({
