...
class knownObjects {
        ...
        .then(snapshot => {
            snapshot.forEach((doc) => {
                if( arrayList.includes(doc.id ) ) {
                    ...
                    case "тварь зоны" :
                        if( doc.data()["is_visible"] ) {
                            coordList.push({"type" : "тварь Зоны", "name": doc.data()["callsign"], "coordinates": doc.data()["coordinates"]});
                        }
                        break;
                    default:
    ...
    async addKnownEntity( id ) {
        ...
        if( this.knownList ) {
            ...
            // again creating a collection
        }
        const newObjRef = firestore().collection('entities/'+this.id+'/knows').doc(id).set({})
        .then( function() {
            console.log( "Added to known : " + my_id + ' : ' + id );
        })
        .catch( function( error ) {
            console.log( "ERROR: cannot add to known : " + my_id + ' : ' + id );
        });
    }
